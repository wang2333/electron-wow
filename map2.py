...
def update_image():
    global big_image, screenshot_np, x, y, width, height, offset, window, show_trace, trace_points, center_x, center_y, movement_speed

    new_screenshot = capture_screen_region(x, y, width, height)
...
def main():
    global big_image, screenshot_np, x, y, width, height, label, offset, window, show_trace, trace_points, center_x, center_y, movement_speed

    # 这里需要手动设置截图区域的坐标和大小
    x, y, width, height = 0, 300, 300, 300
    ...
    screenshot_np = preprocess_image(screenshot)  # 应用修改后的预处理函数
